...
//=================_|===========================================================
    ...
    address     constant private    fundAddr_ = ; // foundation address
	uint256 	constant private 	UNIT = 100000000;         								// unit
    ...
    MillionaireInterface constant private millionaire_ = MillionaireInterface();
    MilAuthInterface constant private milAuth_ = MilAuthInterface();
...
//==============================================================================
    ...
    modifier isEffectiveTime() {
        require(now < round_[rID_].roundDeadline - entertainedTime, "not effective game time");
        _;
    }
     ...
     */
    function drawGame(string btcBlockHash, uint256 drawCode, uint256[3] winNums)
        public
        isActivated()
        onlyDevs()
    {
        require(now > round_[rID_].roundDeadline, "the game is not over");
     ...
     */
    function rewardWinner(address _addr, uint256[] _rWinnerNum)
        public
        isActivated()
        isHuman()
    {
    ...
    {
        mapping(uint256 => uint256) uTickMap = round_[_rid].playerRound[_addr].tickMap;
        ...
        for (uint256 j = 0; j < index; j++) {
            ...
            userNotes[j] = uTickMap[tmpTicks[j]];
        }
    ...
    function getHistoryRoundInfo(uint256 _rid)
        public
        view
        returns(string, uint256[] memory items, address[], address[], address[])
    {
